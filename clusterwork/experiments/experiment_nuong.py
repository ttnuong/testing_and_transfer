...
class ConvAE_Feat(PytorchExperiment):
    ...
    def train(self, epoch):
        ...
        for batch_idx, data in enumerate(self.train_loader):
            ...
            data = data.cuda()
            target = data.cuda() #compare to input
...
class ConvAE_CIFAR(PytorchExperiment):
    ...
    def train(self, epoch):
        ...
        for batch_idx, (data,target) in enumerate(self.train_loader):
            ...
            loss1 = self.criterion(output, data)*float(self.config.run.loss1)
            loss2 = (1-self.criterion2(output, data))*float(self.config.run.loss2)
